...
namespace DynamicVisualizer
{
    internal class FigureScaler
    {
        ...
        public TransformStep NowScaling;
            ...
            switch (selected.Type)
            {
                case Figure.FigureType.Rect:
                    ...
                    if (NowScaling == null)
                    {
                        ...
                        if (p.X < smallW)
                            NowScaling = new ScaleRectStep(rf, ScaleRectStep.Side.Right,
                                1 - (pos.X - _downPos.X)/rf.Width.CachedValue.AsDouble);
                        else if (p.X > 5.0*smallW)
                            NowScaling = new ScaleRectStep(rf, ScaleRectStep.Side.Left,
                                1 + (pos.X - _downPos.X)/rf.Width.CachedValue.AsDouble);
                        else if (p.Y < smallH)
                            NowScaling = new ScaleRectStep(rf, ScaleRectStep.Side.Bottom,
                                1 - (pos.Y - _downPos.Y)/rf.Height.CachedValue.AsDouble);
                        else if (p.Y > 5.0*smallH)
                            NowScaling = new ScaleRectStep(rf, ScaleRectStep.Side.Top,
                                1 + (pos.Y - _downPos.Y)/rf.Height.CachedValue.AsDouble);
                        if (NowScaling == null) return;
                        Timeline.Insert(NowScaling, Timeline.CurrentStepIndex == -1 ? 0 : Timeline.CurrentStepIndex + 1);
                    }
                    else
                    {
                        switch (NowScaling.StepType)
                        {
                            case TransformStep.TransformStepType.ScaleRect:
                                var srs = (ScaleRectStep) NowScaling;
                ...
                case Figure.FigureType.Circle:
                    ...
                    if (NowScaling == null)
                    {
                        ...
                        if (p.X < smallW)
                            NowScaling = new ScaleEllipseStep(ef, ScaleEllipseStep.Side.Right,
                                1 - (pos.X - _downPos.X)/ef.Radius1.CachedValue.AsDouble);
                        else if (p.X > 5.0*smallW)
                            NowScaling = new ScaleEllipseStep(ef, ScaleEllipseStep.Side.Left,
                                1 + (pos.X - _downPos.X)/ef.Radius1.CachedValue.AsDouble);
                        else if (p.Y < smallH)
                            NowScaling = new ScaleEllipseStep(ef, ScaleEllipseStep.Side.Bottom,
                                1 + (pos.Y - _downPos.Y)/ef.Radius2.CachedValue.AsDouble);
                        else if (p.Y > 5.0*smallH)
                            NowScaling = new ScaleEllipseStep(ef, ScaleEllipseStep.Side.Top,
                                1 - (pos.Y - _downPos.Y)/ef.Radius2.CachedValue.AsDouble);
                        if (NowScaling == null) return;
                        Timeline.Insert(NowScaling, Timeline.CurrentStepIndex == -1 ? 0 : Timeline.CurrentStepIndex + 1);
                    }
                    else
                    {
                        switch (NowScaling.StepType)
                        {
                            case TransformStep.TransformStepType.ScaleEllipse:
                                var srs = (ScaleEllipseStep) NowScaling;
